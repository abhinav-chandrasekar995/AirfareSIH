...
### D-084 — CI failure #7, the actual final one: CI's Postgres role was a superuser, unconditionally bypassing RLS - `pytest-rerunfailures` (D-083) was masking the wrong problem
**What changed the picture.** D-083's mitigation shipped and was watched live: `2 failed, 98 passed, 4 rerun` - `pytest-rerunfailures` genuinely retried both tests twice each, and they failed identically every time. That single fact broke the "rare local-pool race" theory D-083 had settled for: a real race would have had a chance of passing on at least one of four retries. Deterministic-within-CI, zero-reproductions-in-six-local-attempts is a different shape of bug entirely - something structurally different about that specific environment, not timing.
**Diagnosis, this time by asking Postgres directly instead of the application.** Added a temporary CI step running raw `psql` - zero Python, zero SQLAlchemy, zero asyncpg - right after the seed load: `SET app.role='PUBLIC'; SELECT count(*) FROM api_keys;` returned **3**. So did the same query with no role set at all. `relforcerowsecurity` was confirmed `t` and the policy existed exactly as expected. The only thing that makes `FORCE ROW LEVEL SECURITY` a no-op regardless of policies is the connecting role being a **superuser** - Postgres never applies RLS to superusers, full stop, no override. `db/rls.py`'s own docstring states the application "connects as a NOSUPERUSER, NOBYPASSRLS role" - true on this machine (`SELECT rolsuper, rolbypassrls FROM pg_roles WHERE rolname='airfare'` -> `f, f`, checked directly to confirm), because that role was created outside Docker at some point before this session. It was never true in CI: `ci.yml`'s Postgres service sets `POSTGRES_USER: airfare` on the stock `timescale/timescaledb` image, and the official Postgres Docker image's entrypoint creates that env var's user as the container's bootstrap superuser - standard, documented behavior of the image, not a misconfiguration anyone wrote by hand. Every one of D-078 through D-083's diagnoses happened to be real, separate bugs found along the way (a stale lint error, a missing dependency, a missing fallback, a missing pytest-asyncio setting, a missing env var capture) - none of them were ever going to touch this, because this bug lived one layer beneath all of them: the CI database itself was never actually enforcing the security boundary the tests exist to check.
**Fix.** Added a step immediately after seed load, before anything else touches the database: `ALTER ROLE airfare WITH NOSUPERUSER NOBYPASSRLS;` via `psql`, executed while still (briefly) a superuser, which Postgres permits - a superuser may revoke its own future superuser status; the change takes effect for new connections/sessions, which is exactly what pytest's connection pool opens fresh for the test run. Placed deliberately *after* migrations and seed load rather than before, so the already-proven-working setup path (which may benefit from broader privileges, e.g. `CREATE EXTENSION timescaledb`) is left untouched, and only the part actually under test - RLS enforcement - runs under the restricted role a real deployment (this project's own Supabase path included) would always use.
**Left in place for one more run: the D-083 diagnostic psql step**, now expected to show `0` for the PUBLIC/no-role checks instead of `3` - the cheapest possible confirmation that the fix works, before even reaching pytest. To be removed once confirmed. The `pytest-rerunfailures`/`@pytest.mark.flaky` marks from D-083 are left in place rather than reverted - they were a reasonable stopgap given what was known at the time, cost nothing now that the real bug is fixed, and only matter if this class of environment drift recurs.

**Correction, same entry, minutes later: the first fix attempt was itself wrong.** Pushed the `ALTER ROLE airfare WITH NOSUPERUSER` step above; CI failed at that exact step: `ERROR: permission denied to alter role / DETAIL: The bootstrap user must have the SUPERUSER attribute.` Postgres refuses to let a cluster's bootstrap superuser strip its own superuser status, precisely so a cluster can never end up with zero superusers - `airfare` in this CI service *is* that bootstrap user (initdb ran with `-U airfare` because `POSTGRES_USER: airfare` was set), so self-alteration was never going to work. This also ruled out an alternative fix of finding some other superuser to do it from - there isn't one; `airfare` is the only superuser this cluster has.
**Actual fix.** Rather than trying to strip privileges from the bootstrap role, create a genuinely separate, properly-restricted role and route the whole pipeline through it instead: added a `Create restricted app role (airfare_app)` step, run before anything touches the database, connecting as the bootstrap `airfare` superuser to run an idempotent `DO $$ ... CREATE ROLE airfare_app WITH LOGIN PASSWORD 'airfare' NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE ... $$;`, then `ALTER SCHEMA public OWNER TO airfare_app` (PG15+ no longer grants `CREATE` on `public` to the `PUBLIC` pseudo-role by default, so ownership - not a grant - is what actually lets migrations create tables as this role) and `GRANT ALL PRIVILEGES ON DATABASE airfare_test TO airfare_app`. `DATABASE_URL`/`TEST_DATABASE_URL` in the job's `env:` block now point at `airfare_app`, not `airfare`, so migrations, seed load, and tests all run as the restricted role from the start - exactly the role shape a real deployment (this project's own Supabase path included) always has, not a privilege carve-out invented for CI. The D-083 diagnostic step's own queries were updated to connect `-U airfare_app` too, so its next run checks the actual role the app uses, not the bootstrap superuser. Applied the identical fix to `e2e-network-off.yml`, which has the same `POSTGRES_USER: airfare` service definition and was silently exercising admin-gated code paths under the same bypass, even though its own test assertions hadn't yet surfaced it as a visible failure.
**Verified locally, to the extent bootstrap-role creation can be:** confirmed by direct query that this machine's own long-lived `airfare` role is correctly `rolsuper=f, rolbypassrls=f` (why none of this ever reproduced locally), and hand-verified the bash `\$\$` escaping inside the YAML `run:` block produces byte-correct `DO $$ ... $$;` Postgres syntax before trusting it in CI - the one part of this fix that could not be exercised locally at all, since reproducing "a fresh cluster whose bootstrap user is named airfare" requires the exact Docker entrypoint behavior CI's service block invokes, not available in this sandbox. The next CI run is the actual test.
**Confirmed, by an actual CI run.** Pushed; watched both workflows with `gh run watch` to completion. `CI / backend`: every step green, including "Run tests" (all previously-failing tests passing, no reruns needed). `Demo resilience (network-off e2e)`: fully green. The D-083 diagnostic step's own output, read back with `gh run view --log`, printed exactly `0`, `0`, `3` for the PUBLIC-role / no-role / ADMIN-role checks respectively - the precise expected pattern, confirmed via raw SQL before pytest even ran. Removed the diagnostic step from `ci.yml` in a follow-up commit now that it had done its job. This closes out the CI-failure sequence that ran from D-078 through this entry: five real, distinct, previously-undiscovered bugs (a stale lint error, a missing `psycopg2` dependency, a missing hypertable-creation fallback, a missing pytest-asyncio setting, and - the one that actually mattered most - a CI database silently running as a superuser and bypassing the exact security boundary its own test suite existed to verify), each found by getting real evidence (an actual log, a raw SQL query, a hand-derivation of transaction semantics) rather than accepting the first plausible-sounding theory.
