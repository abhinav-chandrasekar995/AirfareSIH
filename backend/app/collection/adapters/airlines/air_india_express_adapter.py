"""Air India Express (IX) airline-direct adapter.
...
class AirIndiaExpressAdapter(BaseSourceAdapter):
    source_code = "airindiaexpress"
    ...
    def parse(self, payload: RawPayload, query: FareQuery) -> list[dict]:
        return parse_fare_cards(
            payload=payload, query=query, source_code=self.source_code, airline_code="IX"
        )
